...
def test_create_notification_should_reject_if_missing_required_fields(
    ...
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocked = mocker.patch(f"app.celery.provider_tasks.deliver_{template_type}.apply_async")
            data = {}
...
def test_create_template_raises_invalid_request_when_content_too_large(sample_service, template_type, should_error):
    ...
    try:
        ...
        assert e.message == {
            "content": [f"Content has a character count greater than the limit of {SMS_CHAR_COUNT_LIMIT}"]
        }

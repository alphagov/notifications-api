...
def test_create_report_request_by_type_returns_existing_request(admin_request, sample_service, sample_user, caplog):
    ...
    assert (
        f"Report request {existing_request.id} for user {sample_user.id} (service {sample_service.id})"
        f" with params {json.dumps(expected_params, separators=(',', ':'))}" in caplog.messages
    )
...
def test_create_report_request_by_type_raises_and_logs_value_error(
    admin_request, sample_service, sample_user, mocker, caplog
):
    expected_params = {"notification_type": "email", "notification_status": "failed"}

    mock_create = mocker.patch("app.service.rest.dao_create_report_request")
    mock_create.side_effect = ValueError("something bad happened")
    ...
    expected_log = (
        f"Report request failed for user {sample_user.id} (service {sample_service.id}) with params "
        f"{json.dumps(expected_params, separators=(',', ':'))}: something bad happened"
    )

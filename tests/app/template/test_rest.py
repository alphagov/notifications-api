...
def test_update_languages_should_update_a_template(client, sample_user):
    service = create_service(service_permissions=[LETTER_TYPE])
    template = create_template(service, template_type="letter", postage="second")
    ...
    data = {"languages": "welsh_then_english"}
    data = json.dumps(data)
    auth_header = create_admin_authorization_header()
    ...
    assert update_response.status_code == 200
    update_json_resp = json.loads(update_response.get_data(as_text=True))
    assert update_json_resp["data"]["languages"] == "welsh_then_english"

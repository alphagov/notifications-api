...
@pytest.mark.parametrize(
    "models_module,expected_bind_key",
    (
        (models, None),
        (models.bulk, "bulk"),
    ),
...
def test_fetch_billing_data_for_today_includes_data_with_the_right_key_type(
    notify_db_session, models_module, expected_bind_key
):
    ...
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(today.date(), models_module=models_module)
...
def test_fetch_billing_data_for_day_only_calls_query_for_permission_type(
    notify_db_session, notification_type, models_module, expected_bind_key
):
    ...
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(
            process_day=today.date(), check_permissions=True, models_module=models_module
        )
...
def test_fetch_billing_data_for_day_only_calls_query_for_all_channels(
    ...
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(
            process_day=today.date(), check_permissions=False, models_module=models_module
        )
...
def test_fetch_billing_data_for_today_includes_data_with_the_right_date(
    ...
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(day_under_test.date(), models_module=models_module)
...
def test_fetch_billing_data_for_day_is_grouped_by_service(notify_db_session, models_module, expected_bind_key):
...
def test_fetch_billing_data_for_day_is_grouped_by_provider(notify_db_session, models_module, expected_bind_key):
    service = create_service()
...
def test_fetch_billing_data_for_day_is_grouped_by_rate_mulitplier(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    template = create_template(service=service)
...
def test_fetch_billing_data_for_day_is_grouped_by_international(notify_db_session, models_module, expected_bind_key):
    service = create_service()
...
def test_fetch_billing_data_for_day_groups_by_postage(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    letter_template = create_template(service=service, template_type="letter")
    email_template = create_template(service=service, template_type="email")
...
def test_fetch_billing_data_for_day_groups_by_sent_by(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    letter_template = create_template(service=service, template_type="letter")
    email_template = create_template(service=service, template_type="email")
...
def test_fetch_billing_data_for_day_groups_by_page_count(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    letter_template = create_template(service=service, template_type="letter")
    email_template = create_template(service=service, template_type="email")
...
def test_fetch_billing_data_for_day_returns_empty_list(notify_db_session, models_module, expected_bind_key):
    today = convert_utc_to_bst(datetime.utcnow())
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(today.date(), models_module=models_module)
...
def test_fetch_billing_data_for_day_uses_correct_table(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    ...
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(
            process_day=five_days_ago.date(), service_ids=[service_id], models_module=models_module
        )
...
def test_fetch_billing_data_for_day_returns_list_for_given_service(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    service_2 = create_service(service_name="Service 2")
    template = create_template(service=service)
    ...
    with QueryRecorder() as query_recorder:
        results = fetch_billing_data_for_day(
            process_day=today.date(), service_ids=[service_id], models_module=models_module
        )
...
def test_fetch_billing_data_for_day_bills_correctly_for_status(notify_db_session, models_module, expected_bind_key):
    service = create_service()
    sms_template = create_template(service=service, template_type="sms")
    email_template = create_template(service=service, template_type="email")
    letter_template = create_template(service=service, template_type="letter")

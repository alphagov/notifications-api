...
def test_should_not_allow_duplicate_key_names_per_service(sample_api_key, fake_uuid):
    api_key = ApiKey(
        id=fake_uuid, service=sample_api_key.service, name=sample_api_key.name, created_by=sample_api_key.created_by, key_type=KEY_TYPE_NORMAL
    )
...
def test_save_api_key_can_create_key_with_same_name_if_other_is_expired(sample_service):
    expired_api_key = ApiKey(
        service=sample_service, name="normal api key", created_by=sample_service.created_by, key_type=KEY_TYPE_NORMAL, expiry_date=datetime.utcnow()
    )
...
def test_should_not_return_revoked_api_keys_older_than_7_days(sample_service, days_old, expected_length):
    expired_api_key = ApiKey(
        service=sample_service, name=sample_service.name, created_by=sample_service.created_by, key_type=KEY_TYPE_NORMAL, expiry_date=datetime.utcnow() - timedelta(days=days_old)
    )
    save_model_api_key(expired_api_key)

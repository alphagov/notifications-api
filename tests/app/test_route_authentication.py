def test_all_routes_have_authentication(client):
    ...
    before_req_funcs = {x for x in client.application.before_request_funcs if x is not None}

    blueprint_names = set(client.application.blueprints.keys())
    assert blueprint_names == before_req_funcs

    routes_blueprint_names = {x.split(".")[0] for x in client.application.view_functions.keys()}

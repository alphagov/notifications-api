...
revision = "0504_add_private_pharmacy_org"
...
def upgrade():
    op.execute(
        "INSERT INTO organisation_types(name, is_crown) VALUES('private_pharmacy', false)"
    )
    op.execute(
        "INSERT INTO default_annual_allowance(id, valid_from_financial_year_start, organisation_type, allowance, notification_type) "
        f"VALUES('{uuid.uuid4()}', 2025, 'private_pharmacy', 0, 'sms')"
    )

...
SQLALCHEMY_DATABASE_URI="postgresql://localhost/migration_test" flask db downgrade

# make sure downgrade has left everything okay
echo -e "${BOLDGREEN}============ running upgrade again ============${ENDCOLOR}"
SQLALCHEMY_DATABASE_URI="postgresql://localhost/migration_test" flask db upgrade $CURRENT_VERSION

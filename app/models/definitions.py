...
from sqlalchemy import CheckConstraint, Column, Index, String, UniqueConstraint, and_, func
...
def create_all_models(db_obj):
    class User(db_obj.Model):
        __tablename__ = "users"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        name = Column(db.String, nullable=False, unique=False)
        email_address = Column(db.String(255), nullable=False, index=True, unique=True)
        created_at = Column(db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, index=False, unique=False, nullable=True, onupdate=datetime.datetime.utcnow)
        _password = Column(db.String, index=False, unique=False, nullable=False)
        mobile_number = Column(db.String, index=False, unique=False, nullable=True)
        ...
        failed_login_count = Column(db.Integer, nullable=False, default=0)
        state = Column(db.String, nullable=False, default="pending")
        platform_admin = Column(db.Boolean, nullable=False, default=False)
        current_session_id = Column(UUID(as_uuid=True), nullable=True)
        auth_type = Column(db.String, db.ForeignKey("auth_type.name"), nullable=False, default=SMS_AUTH_TYPE)
        email_access_validated_at = Column(
            db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow
        )
        take_part_in_research = Column(db.Boolean, nullable=False, default=True)
        receives_new_features_email = Column(db.Boolean, nullable=False, default=True)
    ...
    class ServiceUser(db_obj.Model):
        __tablename__ = "user_to_service"
        user_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), primary_key=True)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), primary_key=True)

        __table_args__ = (UniqueConstraint("user_id", "service_id", name="uix_user_to_service"),)

    user_to_organisation = db_obj.Table(
        "user_to_organisation",
        db_obj.Model.metadata,
        db_obj.Column("user_id", UUID(as_uuid=True), db_obj.ForeignKey("users.id")),
        db_obj.Column("organisation_id", UUID(as_uuid=True), db_obj.ForeignKey("organisation.id")),
        UniqueConstraint("user_id", "organisation_id", name="uix_user_to_organisation"),
    )

    user_folder_permissions = db_obj.Table(
        "user_folder_permissions",
        db_obj.Model.metadata,
        Column("user_id", UUID(as_uuid=True), primary_key=True),
        Column("template_folder_id", UUID(as_uuid=True), db.ForeignKey("template_folder.id"), primary_key=True),
        ...
        db.ForeignKeyConstraint(
            ["template_folder_id", "service_id"], ["template_folder.id", "template_folder.service_id"]
        ),
    )

    class BrandingTypes(db_obj.Model):
        __tablename__ = "branding_type"
        name = Column(db.String(255), primary_key=True)

    class EmailBranding(db_obj.Model):
        __tablename__ = "email_branding"
        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        colour = Column(db.String(7), nullable=True)
        logo = Column(db.String(255), nullable=True)
        name = Column(db.String(255), unique=True, nullable=False)
        text = Column(db.String(255), nullable=True)
        alt_text = Column(db.String(255), nullable=True)
        brand_type = Column(
            db.String(255), db.ForeignKey("branding_type.name"), index=True, nullable=False, default=BRANDING_ORG
        )
        created_at = Column(db.DateTime, nullable=True, default=lambda: datetime.datetime.utcnow())
        created_by = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)
        updated_at = Column(db.DateTime, nullable=True, onupdate=lambda: datetime.datetime.utcnow())
        updated_by = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)
    ...
    service_email_branding = db_obj.Table(
        "service_email_branding",
        db_obj.Model.metadata,
        # service_id is a primary key as you can only have one email branding per service
        Column("service_id", UUID(as_uuid=True), db.ForeignKey("services.id"), primary_key=True, nullable=False),
        Column("email_branding_id", UUID(as_uuid=True), db.ForeignKey("email_branding.id"), nullable=False),
    )

    class LetterBranding(db_obj.Model):
        __tablename__ = "letter_branding"
        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        name = Column(db.String(255), unique=True, nullable=False)
        filename = Column(db.String(255), unique=True, nullable=False)
        created_at = Column(db.DateTime, nullable=True, default=datetime.datetime.utcnow)
        created_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
        updated_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)
    ...
    service_letter_branding = db_obj.Table(
        "service_letter_branding",
        db_obj.Model.metadata,
        # service_id is a primary key as you can only have one letter branding per service
        Column("service_id", UUID(as_uuid=True), db.ForeignKey("services.id"), primary_key=True, nullable=False),
        Column("letter_branding_id", UUID(as_uuid=True), db.ForeignKey("letter_branding.id"), nullable=False),
    )

    class ServicePermissionTypes(db_obj.Model):
        __tablename__ = "service_permission_types"

        name = Column(db.String(255), primary_key=True)

    class Domain(db_obj.Model):
        __tablename__ = "domain"
        domain = Column(db.String(255), primary_key=True)
        organisation_id = Column(
            "organisation_id", UUID(as_uuid=True), db.ForeignKey("organisation.id"), nullable=False
        )

    class OrganisationTypes(db_obj.Model):
        __tablename__ = "organisation_types"

        name = Column(db.String(255), primary_key=True)
        is_crown = Column(db.Boolean, nullable=True)

    class OrganisationPermission(db_obj.Model):
        ...
        organisation_id = Column(UUID(as_uuid=True), db.ForeignKey("organisation.id"), nullable=False)
        permission = Column(
    ...
    class OrganisationUserPermissions(db_obj.Model):
        __tablename__ = "organisation_user_permissions"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        created_at = Column(db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow)
        organisation_id = Column(UUID(as_uuid=True), db.ForeignKey("organisation.id"), index=True)
        organisation = db.relationship("Organisation")

        user_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=False)
    ...
    class Organisation(db_obj.Model):
        __tablename__ = "organisation"
        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=False)
        name = Column(db.String(255), nullable=False, unique=True, index=True)
        active = Column(db.Boolean, nullable=False, default=True)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
        agreement_signed = Column(db.Boolean, nullable=True)
        agreement_signed_at = Column(db.DateTime, nullable=True)
        agreement_signed_by_id = Column(
            UUID(as_uuid=True),
            db.ForeignKey("users.id"),
            nullable=True,
        )
        agreement_signed_by = db.relationship("User")
        agreement_signed_on_behalf_of_name = Column(db.String(255), nullable=True)
        agreement_signed_on_behalf_of_email_address = Column(db.String(255), nullable=True)
        agreement_signed_version = Column(db.Float, nullable=True)
        crown = Column(db.Boolean, nullable=True)
        ...
        request_to_go_live_notes = Column(db.Text)
        can_approve_own_go_live_requests = Column(db.Boolean, default=False, nullable=False)
        ...
        purchase_order_number = Column(db.String(255), nullable=True)
        billing_contact_names = Column(db.Text, nullable=True)
        billing_contact_email_addresses = Column(db.Text, nullable=True)
        billing_reference = Column(db.String(255), nullable=True)
    ...
    class OrganisationEmailBranding(db_obj.Model):
        __tablename__ = "email_branding_to_organisation"
        organisation_id = Column(UUID(as_uuid=True), db.ForeignKey("organisation.id"), primary_key=True)
        email_branding_id = Column(UUID(as_uuid=True), db.ForeignKey("email_branding.id"), primary_key=True)

        __table_args__ = (
            UniqueConstraint("organisation_id", "email_branding_id", name="uix_email_branding_to_organisation"),
        )

    letter_branding_to_organisation = db_obj.Table(
    ...
    class Service(db_obj.Model, Versioned):
        __tablename__ = "services"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        _name = Column("name", db.String(255), nullable=False, unique=True)

        # this isn't intended to be accessed, just used for checking service name uniqueness. See `email_sender_local_part`
        _normalised_service_name = Column("normalised_service_name", db.String, nullable=False, unique=True)
        _custom_email_sender_name = Column("custom_email_sender_name", db.String(255), nullable=True)
        _email_sender_local_part = Column("email_sender_local_part", db.String(255), nullable=False)

        created_at = Column(db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, index=False, unique=False, nullable=True, onupdate=datetime.datetime.utcnow)
        active = Column(db.Boolean, index=False, unique=False, nullable=False, default=True)
        restricted = Column(db.Boolean, index=False, unique=False, nullable=False)
        created_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=False)
        created_by = db.relationship("User", foreign_keys=[created_by_id])
        prefix_sms = Column(db.Boolean, nullable=False, default=False)
        organisation_type = Column(
            db.String(255),
            db.ForeignKey("organisation_types.name"),
            unique=False,
        ...
        crown = Column(db.Boolean, index=False, nullable=True)
        rate_limit = Column(db.Integer, index=False, nullable=False, default=3000)
        contact_link = Column(db.String(255), nullable=True, unique=False)

        letter_message_limit = Column(db.BigInteger, index=False, unique=False, nullable=False, default=999_999_999)
        sms_message_limit = Column(db.BigInteger, index=False, unique=False, nullable=False, default=999_999_999)
        international_sms_message_limit = Column(
            db.BigInteger, index=False, unique=False, nullable=False, default=250_000
        )
        email_message_limit = Column(db.BigInteger, index=False, unique=False, nullable=False, default=999_999_999)

        volume_sms = Column(db.Integer(), nullable=True, unique=False)
        volume_email = Column(db.Integer(), nullable=True, unique=False)
        volume_letter = Column(db.Integer(), nullable=True, unique=False)

        consent_to_research = Column(db.Boolean, nullable=True)

        count_as_live = Column(db.Boolean, nullable=False, default=True)
        go_live_user_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)
        go_live_user = db.relationship("User", foreign_keys=[go_live_user_id])
        go_live_at = Column(db.DateTime, nullable=True)
        has_active_go_live_request = Column(db.Boolean, default=False, nullable=False)
        confirmed_unique = Column(db.Boolean, default=False, nullable=False)

        organisation_id = Column(UUID(as_uuid=True), db.ForeignKey("organisation.id"), index=True, nullable=True)
    ...
    class DefaultAnnualAllowance(db_obj.Model):
        ...
        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        valid_from_financial_year_start = Column(db.Integer, index=True, nullable=False)
        organisation_type = Column(
            db.String(255),
            db.ForeignKey("organisation_types.name"),
            unique=False,
        ...
        allowance = Column(db.Integer, nullable=False)
        notification_type = Column(notification_types, index=True, nullable=False)
    ...
    class AnnualBilling(db_obj.Model):
        __tablename__ = "annual_billing"
        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=False, index=True, nullable=False)
        financial_year_start = Column(db.Integer, nullable=False, default=True, unique=False)
        free_sms_fragment_limit = Column(db.Integer, nullable=False, index=False, unique=False)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        high_volume_service_last_year = Column(db.Boolean, unique=False, default=False, nullable=False)
        has_custom_allowance = Column(db.Boolean, unique=False, default=False, nullable=False)
    ...
    class InboundNumber(db_obj.Model):
        __tablename__ = "inbound_numbers"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        number = Column(db.String(11), unique=True, nullable=False)
        provider = Column(db.String(), nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=True, index=True, nullable=True)
    ...
    class ServiceSmsSender(db_obj.Model):
        __tablename__ = "service_sms_senders"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        sms_sender = Column(db.String(11), nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, nullable=False, unique=False)
        service = db.relationship(Service, backref=db.backref("service_sms_senders", uselist=True))
        is_default = Column(db.Boolean, nullable=False, default=True)
        archived = Column(db.Boolean, nullable=False, default=False)
        inbound_number_id = Column(
    ...
    class ServicePermission(db_obj.Model):
    ...
    class ServiceGuestList(db_obj.Model):
        __tablename__ = "service_whitelist"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, nullable=False)
        service = db.relationship("Service", backref="guest_list")
        recipient_type = Column(guest_list_recipient_types, nullable=False)
        recipient = Column(db.String(255), nullable=False)
    ...
    class ServiceCallbackApi(db_obj.Model, Versioned):
        ...
        callback_type = Column(db.String(), db.ForeignKey("service_callback_type.name"), nullable=True)
        _bearer_token = Column("bearer_token", db.String(), nullable=False)
        created_at = Column(db.DateTime, default=datetime.datetime.utcnow, nullable=False)
        updated_at = Column(db.DateTime, nullable=True)
        updated_by = db.relationship("User")
        updated_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=False)
    ...
    class ServiceCallbackType(db_obj.Model):
        __tablename__ = "service_callback_type"

        name = Column(db.String, primary_key=True)

    class ApiKey(db_obj.Model, Versioned):
        __tablename__ = "api_keys"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        name = Column(db.String(255), nullable=False)
        _secret = Column("secret", db.String(255), unique=True, nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, nullable=False)
        service = db.relationship("Service", backref="api_keys")
        key_type = Column(db.String(255), db.ForeignKey("key_types.name"), nullable=False)
    ...
    class KeyTypes(db_obj.Model):
        __tablename__ = "key_types"

        name = Column(db.String(255), primary_key=True)

    class TemplateFolder(db_obj.Model):
        __tablename__ = "template_folder"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), nullable=False)
        name = Column(db.String, nullable=False)
        parent_id = Column(UUID(as_uuid=True), db.ForeignKey("template_folder.id"), nullable=True)
    ...
    template_folder_map = db_obj.Table(
        "template_folder_map",
        db_obj.Model.metadata,
        # template_id is a primary key as a template can only belong in one folder
        Column("template_id", UUID(as_uuid=True), db.ForeignKey("templates.id"), primary_key=True, nullable=False),
        Column("template_folder_id", UUID(as_uuid=True), db.ForeignKey("template_folder.id"), nullable=False),
    )
    ...
    class TemplateBase(db_obj.Model):
        __abstract__ = True
        ...
        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        name = Column(db.String(255), nullable=False)
        template_type = Column(template_types, nullable=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, onupdate=datetime.datetime.utcnow)
        content = Column(db.Text, nullable=False)
        archived = Column(db.Boolean, nullable=False, default=False)
        hidden = Column(db.Boolean, nullable=False, default=False)
        subject = Column(db.Text)
        postage = Column(db.String, nullable=True)

        letter_welsh_content = Column(db.Text)
        letter_welsh_subject = Column(db.Text)
        letter_languages = Column(
        ...
        has_unsubscribe_link = Column(db.Boolean, default=False, nullable=False)

        @declared_attr
        def service_id(cls):
            return Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, nullable=False)

        @declared_attr
        def created_by_id(cls):
            return Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=False)
        ...
        def service_letter_contact_id(cls):
            return Column(UUID(as_uuid=True), db.ForeignKey("service_letter_contacts.id"), nullable=True)
        ...
        def letter_attachment_id(cls):
            return Column(UUID(as_uuid=True), db.ForeignKey("letter_attachment.id"), nullable=True)
        ...
        def __extended_statistics__(cls):
            return (
                # dependencies
                (
                    f"st_dep_{cls.__tablename__}_service_id_ctd_by_id",
                    ("service_id", "created_by_id"),
                    ("dependencies",),
                ),
                (
        ...
        def serialize_for_v2(self):
            serialized = {
                ...
                "letter_contact_block": self.service_letter_contact.contact_block
    ...
    class Template(TemplateBase):
        ...
        version = Column(db.Integer, default=0, nullable=False)
    ...
    class TemplateRedacted(db_obj.Model):
        __tablename__ = "template_redacted"

        template_id = Column(UUID(as_uuid=True), db.ForeignKey("templates.id"), primary_key=True, nullable=False)
        redact_personalisation = Column(db.Boolean, nullable=False, default=False)
        updated_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False, index=True)
        updated_by = db.relationship("User")
    ...
    class TemplateHistory(TemplateBase):
        ...
        version = Column(db.Integer, primary_key=True, nullable=False)
    ...
    class TemplateEmailFileBase(db_obj.Model):
        __abstract__ = True

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)

        filename = Column(db.Text, nullable=False)
        link_text = Column(db.Text, nullable=True)
        retention_period = Column(db.Integer, default=1, nullable=False)
        validate_users_email = Column(db.Boolean, default=True, nullable=False)

        created_at = Column(db.DateTime, nullable=False, onupdate=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
        archived_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)

        @declared_attr
        def template_id(cls):
            return Column(UUID(as_uuid=True), db.ForeignKey("templates.id"), index=True, nullable=False)

        @declared_attr
        def template_version(cls):
            return Column(db.Integer(), index=True, nullable=False)
    ...
    class ProviderDetails(db_obj.Model):
        __tablename__ = "provider_details"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        display_name = Column(db.String, nullable=False)
        identifier = Column(db.String, nullable=False)
        priority = Column(db.Integer, nullable=False)
        notification_type = Column(notification_types, nullable=False)
        active = Column(db.Boolean, default=False, nullable=False)
        version = Column(db.Integer, default=1, nullable=False)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
        created_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=True)
        created_by = db.relationship("User")
        supports_international = Column(db.Boolean, nullable=False, default=False)

    class ProviderDetailsHistory(db_obj.Model):
    ...
    class JobStatus(db_obj.Model):
        __tablename__ = "job_status"

        name = Column(db.String(255), primary_key=True)

    class Job(db_obj.Model):
        __tablename__ = "jobs"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        original_file_name = Column(db.String, nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, unique=False, nullable=False)
        service = db.relationship("Service", backref=db.backref("jobs", lazy="dynamic"))
        template_id = Column(UUID(as_uuid=True), db.ForeignKey("templates.id"), index=True, unique=False)
        template = db.relationship("Template", backref=db.backref("jobs", lazy="dynamic"))
        template_version = Column(db.Integer, nullable=False)
        created_at = Column(db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, index=False, unique=False, nullable=True, onupdate=datetime.datetime.utcnow)
        notification_count = Column(db.Integer, nullable=False)
        notifications_sent = Column(db.Integer, nullable=False, default=0)
        notifications_delivered = Column(db.Integer, nullable=False, default=0)
        notifications_failed = Column(db.Integer, nullable=False, default=0)

        processing_started = Column(db.DateTime, index=False, unique=False, nullable=True)
        processing_finished = Column(db.DateTime, index=False, unique=False, nullable=True)
        created_by = db.relationship("User")
        created_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=True)
        scheduled_for = Column(db.DateTime, index=True, unique=False, nullable=True)
        job_status = Column(
            db.String(255), db.ForeignKey("job_status.name"), index=True, nullable=False, default="pending"
        )
        archived = Column(db.Boolean, nullable=False, default=False)
        contact_list_id = Column(
            UUID(as_uuid=True), db.ForeignKey("service_contact_list.id"), nullable=True, index=True
        )
    ...
    class NotificationStatusTypes(db_obj.Model):
        __tablename__ = "notification_status_types"

        name = Column(db.String(), primary_key=True)

    class NotificationAllTimeView(db_obj.Model):
        """
        ...
        job_row_number = Column(db.Integer)
        service_id = Column(UUID(as_uuid=True))
        template_id = Column(UUID(as_uuid=True))
        template_version = Column(db.Integer)
        api_key_id = Column(UUID(as_uuid=True))
        key_type = Column(db.String)
        billable_units = Column(db.Integer)
        notification_type = Column(notification_types)
        created_at = Column(db.DateTime)
        sent_at = Column(db.DateTime)
        sent_by = Column(db.String)
        updated_at = Column(db.DateTime)
        status = Column("notification_status", db.Text)
        reference = Column(db.String)
        client_reference = Column(db.String)
        international = Column(db.Boolean)
        phone_prefix = Column(db.String)
        rate_multiplier = Column(db.Numeric(asdecimal=False))
        created_by_id = Column(UUID(as_uuid=True))
        postage = Column(db.String)
        document_download_count = Column(db.Integer)

    class Notification(db_obj.Model):
        __tablename__ = "notifications"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        to = Column(db.String, nullable=False)
        normalised_to = Column(db.String, nullable=True)
        job_id = Column(UUID(as_uuid=True), db.ForeignKey("jobs.id"), index=True, unique=False)
        job = db.relationship("Job", backref=db.backref("notifications", lazy="dynamic"))
        job_row_number = Column(db.Integer, nullable=True)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=False)
        service = db.relationship("Service")
        template_id = Column(UUID(as_uuid=True), index=True, unique=False)
        template_version = Column(db.Integer, nullable=False)
        template = db.relationship("TemplateHistory")
        api_key_id = Column(UUID(as_uuid=True), db.ForeignKey("api_keys.id"), unique=False)
        api_key = db.relationship("ApiKey")
        key_type = Column(db.String, db.ForeignKey("key_types.name"), unique=False, nullable=False)
        billable_units = Column(db.Integer, nullable=False, default=0)
        notification_type = Column(notification_types, nullable=False)
        created_at = Column(db.DateTime, index=True, unique=False, nullable=False)
        sent_at = Column(db.DateTime, index=False, unique=False, nullable=True)
        sent_by = Column(db.String, nullable=True)
        updated_at = Column(db.DateTime, index=False, unique=False, nullable=True, onupdate=datetime.datetime.utcnow)
        status = Column(
            "notification_status",
            db.Text,
        ...
        reference = Column(db.String, nullable=True, index=True)
        client_reference = Column(db.String, index=True, nullable=True)
        _personalisation = Column(db.String, nullable=True)

        international = Column(db.Boolean, nullable=False, default=False)
        phone_prefix = Column(db.String, nullable=True)
        rate_multiplier = Column(db.Numeric(asdecimal=False), nullable=True)

        created_by = db.relationship("User")
        created_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)

        reply_to_text = Column(db.String, nullable=True)

        document_download_count = Column(db.Integer, nullable=True)

        postage = Column(db.String, nullable=True)

        unsubscribe_link = Column(db.String, nullable=True)
        ...
        __extended_statistics__ = (
            ...
            (
                "st_dep_notifications_ntfcn_type_doc_dl",
            ...
            letter_rate = float(
                next(
                    (
                        rate
                        for rate in rates
                        if rate.sheet_count == self.billable_units and rate.post_class == self.postage
                    ),
                    None,
                ).rate
            )
    ...
    class NotificationHistory(db_obj.Model):
        ...
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=False)
        service = db.relationship("Service")
        template_id = Column(UUID(as_uuid=True), unique=False)
        template_version = Column(db.Integer, nullable=False)
        api_key_id = Column(UUID(as_uuid=True), db.ForeignKey("api_keys.id"), unique=False)
        api_key = db.relationship("ApiKey")
        key_type = Column(db.String, db.ForeignKey("key_types.name"), unique=False, nullable=False)
        billable_units = Column(db.Integer, nullable=False, default=0)
        notification_type = Column(notification_types, nullable=False)
        created_at = Column(db.DateTime, unique=False, nullable=False)
        sent_at = Column(db.DateTime, index=False, unique=False, nullable=True)
        sent_by = Column(db.String, nullable=True)
        updated_at = Column(db.DateTime, index=False, unique=False, nullable=True, onupdate=datetime.datetime.utcnow)
        status = Column(
        ...
        reference = Column(db.String, nullable=True, index=True)
        client_reference = Column(db.String, nullable=True)

        international = Column(db.Boolean, nullable=True, default=False)
        phone_prefix = Column(db.String, nullable=True)
        rate_multiplier = Column(db.Numeric(asdecimal=False), nullable=True)
    ...
    class InviteStatusType(db_obj.Model):
        __tablename__ = "invite_status_type"

        name = Column(db.String, primary_key=True)

    class InvitedUser(db_obj.Model):
        __tablename__ = "invited_users"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        email_address = Column(db.String(255), nullable=False)
        user_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=False)
        from_user = db.relationship("User")
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, unique=False)
        service = db.relationship("Service")
        created_at = Column(db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow)
        status = Column(
        ...
        permissions = Column(db.String, nullable=False)
        auth_type = Column(
            db.String, db.ForeignKey("auth_type.name"), index=True, nullable=False, default=SMS_AUTH_TYPE
        )
        folder_permissions = Column(JSONB(none_as_null=True), nullable=False, default=[])
    ...
    class InvitedOrganisationUser(db_obj.Model):
        __tablename__ = "invited_organisation_users"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        email_address = Column(db.String(255), nullable=False)
        invited_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False)
        ...
        status = Column(db.String, db.ForeignKey("invite_status_type.name"), nullable=False, default=INVITE_PENDING)
    ...
    class Permission(db_obj.Model):
        __tablename__ = "permissions"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        # Service id is optional, if the service is omitted we will assume the permission is not service specific.
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, unique=False, nullable=True)
        service = db.relationship("Service")
        user_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), index=True, nullable=False)
        user = db.relationship("User")
        permission = Column(
    ...
    class Event(db_obj.Model):
        __tablename__ = "events"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        event_type = Column(db.String(255), nullable=False)
        created_at = Column(db.DateTime, index=False, unique=False, nullable=False, default=datetime.datetime.utcnow)
        data = Column(JSON, nullable=False)

    class Rate(db_obj.Model):
        __tablename__ = "rates"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        valid_from = Column(db.DateTime, nullable=False)
        rate = Column(db.Numeric(asdecimal=False), nullable=False)
        notification_type = Column(notification_types, index=True, nullable=False)
    ...
    class InboundSms(db_obj.Model):
        ...
        notify_number = Column(db.String, nullable=False)  # the service's number, that the msg was sent to
        user_number = Column(db.String, nullable=False, index=True)  # the end user's number, that the msg was sent from
        provider_date = Column(db.DateTime)
        provider_reference = Column(db.String)
        provider = Column(db.String, nullable=False)
        _content = Column("content", db.String, nullable=False)

        __extended_statistics__ = (
            # dependencies
            (
                "st_dep_inb_sms_service_id_ntfy_num_provider",
    ...
    class InboundSmsHistory(db_obj.Model):
        __tablename__ = "inbound_sms_history"
        id = Column(UUID(as_uuid=True), primary_key=True)
        created_at = Column(db.DateTime, unique=False, nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), index=True, unique=False)
        service = db.relationship("Service")
        notify_number = Column(db.String, nullable=False)
        provider_date = Column(db.DateTime)
        provider_reference = Column(db.String)
        provider = Column(db.String, nullable=False)
    ...
    class LetterRate(db_obj.Model):
        __tablename__ = "letter_rates"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        start_date = Column(db.DateTime, nullable=False)
        end_date = Column(db.DateTime, nullable=True)
        sheet_count = Column(db.Integer, nullable=False)  # double sided sheet
        rate = Column(db.Numeric(), nullable=False)
        crown = Column(db.Boolean, nullable=False)
        post_class = Column(db.String, nullable=False)
    ...
    class ServiceEmailReplyTo(db_obj.Model):
        ...
        email_address = Column(db.Text, nullable=False, index=False, unique=False)
        is_default = Column(db.Boolean, nullable=False, default=True)
        archived = Column(db.Boolean, nullable=False, default=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    ...
    class ServiceLetterContact(db_obj.Model):
        ...
        contact_block = Column(db.Text, nullable=False, index=False, unique=False)
        is_default = Column(db.Boolean, nullable=False, default=True)
        archived = Column(db.Boolean, nullable=False, default=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    ...
    class AuthType(db_obj.Model):
        __tablename__ = "auth_type"

        name = Column(db.String, primary_key=True)

    class FactBillingLetterDespatch(db_obj.Model):
        __tablename__ = "ft_billing_letter_despatch"

        bst_date = Column(db.Date, nullable=False, primary_key=True)
        postage = Column(db.String, nullable=False, primary_key=True)
        cost_threshold = Column(
        ...
        rate = Column(db.Numeric(), nullable=False, primary_key=True)
        billable_units = Column(db.Integer(), nullable=True, primary_key=True)
        notifications_sent = Column(db.Integer(), nullable=True)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)

    class FactBilling(db_obj.Model):
        __tablename__ = "ft_billing"

        bst_date = Column(db.Date, nullable=False, primary_key=True, index=True)
        template_id = Column(UUID(as_uuid=True), nullable=False, primary_key=True, index=True)
        service_id = Column(UUID(as_uuid=True), nullable=False, primary_key=True, index=True)
        notification_type = Column(db.Text, nullable=False, primary_key=True)
        provider = Column(db.Text, nullable=False, primary_key=True)
        rate_multiplier = Column(db.Integer(), nullable=False, primary_key=True)
        international = Column(db.Boolean, nullable=False, primary_key=True)
        rate = Column(db.Numeric(), nullable=False, primary_key=True)
        postage = Column(db.String, nullable=False, primary_key=True)
        billable_units = Column(db.Integer(), nullable=True)
        notifications_sent = Column(db.Integer(), nullable=True)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    ...
    class FactNotificationStatus(db_obj.Model):
        __tablename__ = "ft_notification_status"

        bst_date = Column(db.Date, index=True, primary_key=True, nullable=False)
        template_id = Column(UUID(as_uuid=True), primary_key=True, index=True, nullable=False)
        service_id = Column(
            UUID(as_uuid=True),
            primary_key=True,
            index=True,
        ...
        job_id = Column(UUID(as_uuid=True), primary_key=True, index=True, nullable=False)
        notification_type = Column(db.Text, primary_key=True, nullable=False)
        key_type = Column(db.Text, primary_key=True, nullable=False)
        notification_status = Column(db.Text, primary_key=True, nullable=False)
        notification_count = Column(db.Integer(), nullable=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    ...
    class FactProcessingTime(db_obj.Model):
        __tablename__ = "ft_processing_time"

        bst_date = Column(db.Date, index=True, primary_key=True, nullable=False)
        messages_total = Column(db.Integer(), nullable=False)
        messages_within_10_secs = Column(db.Integer(), nullable=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)

    class Complaint(db_obj.Model):
        __tablename__ = "complaints"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        notification_id = Column(UUID(as_uuid=True), index=True, nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=False, index=True, nullable=False)
        service = db.relationship(Service, backref=db.backref("complaints"))
        ses_feedback_id = Column(db.Text, nullable=True)
        complaint_type = Column(db.Text, nullable=True)
        complaint_date = Column(db.DateTime, nullable=True)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
    ...
    class ServiceDataRetention(db_obj.Model):
        ...
        notification_type = Column(notification_types, nullable=False)
        days_of_retention = Column(db.Integer, nullable=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)
    ...
    class ReturnedLetter(db_obj.Model):
        __tablename__ = "returned_letters"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        reported_at = Column(db.Date, nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=False, index=True, nullable=False)
        service = db.relationship(Service, backref=db.backref("returned_letters"))
        notification_id = Column(UUID(as_uuid=True), unique=True, nullable=False)
        created_at = Column(db.DateTime, nullable=False)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)

    class ServiceContactList(db_obj.Model):
        __tablename__ = "service_contact_list"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        original_file_name = Column(db.String, nullable=False)
        row_count = Column(db.Integer, nullable=False)
        template_type = Column(template_types, nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), unique=False, index=True, nullable=False)
            ...
            return (
                ...
                .outerjoin(
                    ...
                    and_(
                        ...
                        func.cast(self.template_type, String)
    ...
    class WebauthnCredential(db_obj.Model):
        """
    ...
    class LetterAttachment(db_obj.Model):
        __tablename__ = "letter_attachment"

        id = Column(UUID(as_uuid=True), primary_key=True, nullable=False, default=uuid.uuid4)

        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        created_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False)
        archived_at = Column(db.DateTime, nullable=True)
        archived_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)

        original_filename = Column(db.String, nullable=False)
        page_count = Column(db.SmallInteger, nullable=False)
    ...
    class UnsubscribeRequestReport(db_obj.Model):
        ...
        created_at = Column(db.DateTime, nullable=True, default=datetime.datetime.utcnow)
        earliest_timestamp = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        latest_timestamp = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        processed_by_service_at = Column(db.DateTime, nullable=True)
        count = Column(db.BigInteger, nullable=False)
    ...
    class UnsubscribeRequest(db_obj.Model):
        ...
        email_address = Column(db.Text, nullable=False)

        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)

        unsubscribe_request_report_id = Column(
            UUID(as_uuid=True), db.ForeignKey("unsubscribe_request_report.id"), index=True, nullable=True
        )
        unsubscribe_request_report = db.relationship(
            UnsubscribeRequestReport, backref=db.backref("unsubscribe_requests")
        )
    ...
    class UnsubscribeRequestHistory(db_obj.Model):
        ...
        template_version = Column(db.Integer, nullable=False)
        created_at = Column(db.DateTime, nullable=False)
        unsubscribe_request_report_id = Column(UUID(as_uuid=True), index=True, nullable=True)
    ...
    class ProtectedSenderId(db_obj.Model):
        __tablename__ = "protected_sender_ids"

        sender_id = Column(db.String, primary_key=True, nullable=False)
    ...
    contacted_users = db_obj.Table(
        ...
        Column("user_id", UUID(as_uuid=True), db.ForeignKey("users.id"), primary_key=True),
    )

    class ServiceJoinRequest(db_obj.Model):
        __tablename__ = "service_join_requests"

        id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
        requester_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False)
        service_id = Column(UUID(as_uuid=True), db.ForeignKey("services.id"), nullable=False)
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        status = Column(
        ...
        status_changed_at = Column(db.DateTime, nullable=True)
        status_changed_by_id = Column(UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True)
        reason = Column(db.Text, nullable=True)
    ...
    class ReportRequest(db_obj.Model):
        ...
        _parameter = Column("parameter", JSONB, nullable=False, default={})
        created_at = Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
        updated_at = Column(db.DateTime, nullable=True, onupdate=datetime.datetime.utcnow)

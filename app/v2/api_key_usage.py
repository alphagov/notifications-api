...
def record_service_api_key_hourly_usage_at_endpoint(api_key_id, service_id, endpoint):
    """
    ...
    hourly_bucket = usage_hour.strftime("%Y%m%d%H")
...
def _record_service_api_key_hourly_usage(api_key_id, service_id, hourly_bucket, endpoint, usage_hour):
    ...
    return RequestCache.CacheResultWrapper(value=1, cache_decision=True)
